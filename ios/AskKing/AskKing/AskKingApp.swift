...
final class AppDelegate: NSObject, UIApplicationDelegate, UNUserNotificationCenterDelegate {
    ...
    func userNotificationCenter(_ center: UNUserNotificationCenter, didReceive response: UNNotificationResponse) async {
        ...
        case NotificationActions.allowAction:
            await appState.decideFromNotification(id: id, decision: "allow")
        case NotificationActions.denyAction:
            await appState.decideFromNotification(id: id, decision: "deny")
        case NotificationActions.replyAction:
            if let textResponse = response as? UNTextInputNotificationResponse {
                await appState.replyFromNotification(id: id, reply: textResponse.userText)
            }
        default:
            await MainActor.run {
                appState.selectedRoute = EventRoute(kind: kind, id: id)
            }
        }
    }
}
...
enum NotificationActions {
    ...
    static func register() {
        ...
        UNUserNotificationCenter.current().setNotificationCategories([approval, completion])
    }
}
